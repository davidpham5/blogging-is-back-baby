...
module.exports = function loadCollection(eleventyConfig) {
	...
	const contentTypes = (collection) =>
		Object.values(
            ...
            timn: {
							id: "timn",
							name: "This Is My Next",
							slug: "this-is-my-next",
							items: [],
						},
            // quickposts: {
            //   id: "quickposts",
	...
	const contentPaginatedByType = (collection) =>
		contentTypes(collection)
			.filter(
				(type) =>
					["project", "resource", "glossary", "amplify", "timn"].includes(type.id) ===
					false
			)
			.reduce(paginateContentTaxonomy(), []);
	...
	const timnPosts = (collection) =>
		post(collection)
			.filter((p) => p.data.contentType === "timn" && !p.fileSlug.endsWith("-todos"))
			.reverse();
