...
const includesTag = (tags, slug) =>
	tags && Array.isArray(tags) ? tags.find((tag) => tag.toLowerCase() === slug.toLowerCase()) !== undefined : false;
...
/**
 * Simple date filter that accepts format strings
...
/**
 * Orders the combined feed in an editorial pattern:
 *   [firstTurboCount turbolinks] → [amplifyCount storystream posts] → [remaining turbolinks]
 *
 * @param {Array} raindropItems  - Raindrop.io article objects
 * @param {Array} amplifyItems   - Eleventy collection items with contentType 'amplify'
 * @param {number} firstTurboCount  - How many turbolinks to show before the storystream break (default 3)
 * @param {number} amplifyCount     - How many storystream posts to inject in the middle (default 5)
 * @returns {Array}
 */
const editorialFeed = (raindropItems, amplifyItems, firstTurboCount = 3, amplifyCount = 5) => {
  const turbo = (raindropItems || []).map(article => ({
    _feedType: 'turbo',
    _date: new Date(article.date),
    article,
  }));
  const amplify = (amplifyItems || []).slice(0, amplifyCount).map(post => ({
    _feedType: 'amplify',
    _date: new Date(post.date),
    post,
  }));
  return [
    ...turbo.slice(0, firstTurboCount),
    ...amplify,
    ...turbo.slice(firstTurboCount),
  ];
};

/**
 * Merges amplify collection items and raindrop turbolink articles into a single
 * date-sorted array for the combined main feed. Each item is tagged with _feedType
 * ('amplify' or 'turbo') and a normalized _date for sorting.
 *
 * @param {Array} amplifyItems - Eleventy collection items with contentType 'amplify'
 * @param {Array} raindropItems - Raindrop.io article objects
 * @returns {Array}
 */
const mergeFeeds = (amplifyItems, raindropItems) => {
  const amplify = (amplifyItems || []).map(post => ({
    _feedType: 'amplify',
    _date: new Date(post.date),
    post,
  }));
  ...
  return [...amplify, ...turbo].sort((a, b) => b._date - a._date);
};
