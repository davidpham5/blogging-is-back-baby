...
async function initCache() {
  console.log(chalk.blue('[@davidpham5/git-history]'), 'Loading Cache');
...
async function getChanges(data) {
  ...
  if (cached) {
    ...
    console.log(chalk.blue('[@davidpham5/git-history]'), 'File content for [' + chalk.green(filePathname) + '] changed, fetching fresh git history');
  } else {
    console.log(chalk.blue('[@davidpham5/git-history]'), 'Fetching fresh git history for: [' + chalk.green(filePathname) + ']');
  }
  ...
  try {
    ...
    const mapped = history.all.map(el => ({
      ...
      url: `https://github.com/davidpham5/davidchicopham.com/commit/${encodeURIComponent(el.hash)}`
    }));
